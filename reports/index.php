...
if (!isset($_GET["report"])) {
  ...
  $report = Report::getReport(intval($_GET["report"]));
  if ($report === false) {
    exit(header("Location: ../"));
  }
  $status = $report::$report_status[$report->getStatus()];
}
?>
...
<body>
  ...
  <div class="container-md">
    <?php if ($report->getUser_id() !== null) {
      echo '<h4 class="text-center">Reported by <a class="text-decoration-none" href="https://forums.3d-sof2.com/members/' . $report->getUser_id() . '/" target="_blank">' . Database::connect()->getUsername($report->getUser_id()) . '</a></h4><hr>';
    }
    ?>
    <div class="card w-100 bg-dark" id="<?php echo $report->getReport_id() ?>" style="margin-bottom: 1.5rem;">
      <div style="background-color: <?php echo $status[1] ?>; font-size: large;" class="card-header text-white text-center">Map: <span data-target="map"><?php echo htmlentities($report->getMap_name()) . ' #' . htmlentities($report->getEntity_id()) ?></span></div>
      <div class="card-body">
        <h5 data-target="title" class="card-title text-center text-white"><?php echo htmlentities($report->getTitle()) ?></h4>
          ...
          <ul class="list-group list-group-flush">
            ...
            <br>
            <p class="text-white card-text col">Status: <span style="color: <?php echo $status[2]; ?>"><?php echo $status[0] ?></span></p>
